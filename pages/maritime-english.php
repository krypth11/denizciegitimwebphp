    ...
    <div class="card-body">
      ...
      <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
          <thead><tr><th style="min-width:130px">Kelime</th><th style="min-width:140px">Kategori</th><th style="min-width:130px">Soru Tipi</th><th style="min-width:320px">Soru ve Seçenekler</th><th>Doğru</th><th>Durum</th><th>Tarih</th></tr></thead>
          <tbody id="meQuestionRows"><tr><td colspan="7" class="text-center text-muted py-4">Yükleniyor...</td></tr></tbody>
        </table>
      </div>
...
<script>
(() => {
 const endpoint='../ajax/maritime-english-import.php'; let parsed=null; let listPage=1; let listLoading=false; let searchTimer=null;
 ...
 const loadQuestions=async(page=1)=>{if(listLoading)return;listLoading=true;const rows=document.querySelector('#meQuestionRows');const err=document.querySelector('#meListError');err.classList.add('d-none');rows.innerHTML='<tr><td colspan="7" class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Yükleniyor...</td></tr>';const qs=new URLSearchParams({action:'list',page:String(page),per_page:document.querySelector('#mePerPage').value,category_id:document.querySelector('#meFilterCategory').value,question_type:document.querySelector('#meFilterType').value,is_active:document.querySelector('#meFilterActive').value,search:document.querySelector('#meFilterSearch').value.trim()});try{const r=await fetch(endpoint+'?'+qs);const j=await r.json();if(!r.ok||!j.success)throw j;const items=j.data.items||[];const p=j.data.pagination;listPage=p.page;document.querySelector('#meListSummary').textContent=`Toplam ${p.total} soru`;document.querySelector('#mePageInfo').textContent=`Sayfa ${p.page} / ${p.total_pages} • Bu sayfada ${items.length} kayıt`;rows.innerHTML=items.length?items.map(x=>{const options=Object.entries(x.options||{}).map(([k,v])=>`<div class="small ${k===x.correct_option_key?'text-success fw-semibold':''}"><span class="badge ${k===x.correct_option_key?'bg-success':'bg-secondary'} me-1">${esc(k)}</span>${esc(v)}</div>`).join('');return `<tr><td><strong>${esc(x.term_en)}</strong><div class="small text-muted">${esc(x.term_tr)}</div></td><td>${esc(x.category_name)}${x.qualification_name?`<div class="small text-muted">${esc(x.qualification_name)}</div>`:''}</td><td><span class="badge text-bg-info">${esc(typeLabel(x.question_type))}</span></td><td><div class="fw-semibold mb-2" style="white-space:pre-wrap">${esc(x.prompt)}</div>${options}</td><td><span class="badge text-bg-success">${esc(x.correct_option_key)}</span></td><td><span class="badge ${Number(x.is_active)===1?'text-bg-success':'text-bg-secondary'}">${Number(x.is_active)===1?'Aktif':'Pasif'}</span></td><td class="small text-nowrap">${esc(x.created_at)}</td></tr>`;}).join(''):'<tr><td colspan="7" class="text-center text-muted py-5">Filtrelere uygun soru bulunamadı.</td></tr>';renderPages(p.page,p.total_pages);}catch(e){rows.innerHTML='<tr><td colspan="7" class="text-center text-danger py-4">Sorular yüklenemedi.</td></tr>';err.textContent=e?.message||'Soru listesi alınamadı.';err.classList.remove('d-none');}finally{listLoading=false;}};
