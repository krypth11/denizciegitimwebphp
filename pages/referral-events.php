...
$(function(){
 ...
 async function load(){ const r=await window.appAjax({url:ep+'?action=list_events',data:{status:$('#statusFilter').val(),search:$('#search').val()},dataType:'json'}); $('#eventsBody').html((r.data.items||[]).map(x=>{ const days=[x.referrer_reward_days,x.referred_reward_days,x.buyer_bonus_days].map(Number).reduce((a,b)=>a+b,0); const fraud=(String(x.status||'')==='suspicious'||String(x.fraud_flags_json||'[]')!=='[]')?'⚠️ '+esc(x.fraud_flags_json||'suspicious'):'-'; return `<tr><td>${esc(x.referrer_name||x.referrer_user_id||'-')}</td><td>${esc(x.referred_name||x.purchase_user_name||x.referred_user_id||x.purchase_user_id||'-')}</td><td><span class="badge text-bg-light border">${esc(x.event_kind)}</span></td><td>${esc(x.product_id)}</td><td>${esc(x.plan_code)}</td><td>${days} gün / +${esc(x.referrer_bonus_percent_delta||0)}%</td><td><small>${esc(x.eligible_at)}</small></td><td>${esc(x.status)}</td><td>${fraud}</td><td class="text-end"><button class="btn btn-sm btn-success act" data-a="approve_event" data-id="${esc(x.id)}">Onayla</button> <button class="btn btn-sm btn-warning act" data-a="reject_event" data-id="${esc(x.id)}">Reddet</button> <button class="btn btn-sm btn-danger act" data-a="reverse_event" data-id="${esc(x.id)}">Geri al</button> <button class="btn btn-sm btn-outline-secondary act" data-a="mark_suspicious" data-id="${esc(x.id)}">Şüpheli</button></td></tr>` }).join('')||'<tr><td colspan="10" class="text-muted">Kayıt yok.</td></tr>'); }
