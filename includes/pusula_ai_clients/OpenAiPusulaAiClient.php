...
class OpenAiPusulaAiClient implements PusulaAiClientInterface
{
    ...
    {
        $provider = strtolower(trim((string)($this->settings['provider'] ?? 'openai')));
        $model = trim((string)($this->settings['model'] ?? 'gpt-5.4-mini'));
    ...
    private function postJson(string $url, array $headers, array $payload, int $timeout, string $provider, string $model): array
    {
        ...
        return $this->result(false, 'OpenAI test hatası (HTTP ' . $httpCode . '): ' . $safeMessage, $provider, $model);
    }
