    ...
    $stmt = $pdo->prepare(
        "SELECT c.id, c.name, c.description, c.sort_order,
                COUNT(DISTINCT CASE WHEN t.is_active = 1 AND t.content_status = 'published'
                  ...
                  AND (SELECT COUNT(*) FROM maritime_english_questions tq WHERE tq.term_id = t.id AND tq.is_active = 1 AND tq.deleted_at IS NULL) >= 2
                  THEN t.id END) AS term_count,
                COUNT(DISTINCT CASE WHEN q.is_active = 1 AND q.deleted_at IS NULL AND t.is_active = 1 AND t.content_status = 'published'
